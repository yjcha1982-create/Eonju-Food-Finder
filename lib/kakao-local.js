...
async function kakaoGet(path, params, apiKey) {
  ...
  if (!response.ok) {
    ...
    const detail = body.msg || body.message || body.error || "";
    const error = new Error(
      `카카오 Local API 오류 (${response.status})${detail ? `: ${detail}` : ""}`,
    );
    error.statusCode = response.status;
    error.kakaoCode = body.code;
    throw error;
  }
...
async function fetchKakaoRestaurants({
  ...
  if (query.trim()) {
    const exactResult = await kakaoGet(
      "/v2/local/search/keyword.json",
      {
        query: query.trim(),
        x: String(longitude),
    ...
    results.push(...(exactResult.documents || []));
  }
